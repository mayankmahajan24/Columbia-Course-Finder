    ...
    Selectpicker.prototype = {
        ...
        createLi: function() {
            ...
            if (!this.multiple && this.$element.find('option:selected').length==0 && !this.options.title) {
                ...
                filtersList[id].push(this.$element.find('option').eq(0).val());
            }
               ...
               //Dont close on multi choice menu
                ...
                if (!that.isDisabled() && !$(this).parent().hasClass('disabled'))       
                {
                    ...
                    //Else toggle the one we have chosen if we are multi select.
                        ...
                        //                       
                        var $selectedOptions = that.$element.find('option').filter(':selected');
                        var selectedArray = [];
                        for (var x = 0; x < $selectedOptions.length; x++)
                        {
                            selectedArray.push($selectedOptions.eq(x).val());
                        }

                        if ($.inArray($option.val(), filtersList[id]) == -1){
                            filtersList[id].push($option.val());
                        }
                        else{
                            filtersList[id].splice($.inArray($option.val(), filtersList[id]), 1);
                        }
                        ...
                        //Handling "All"
                       if ($option.val() == 'All' && prevValue != 'All')
                        {
                             that.deselectAll();
                             $option.prop('selected', true);
                        }
